#!/bin/bash
#to be sourced from run_level3.sh
echo '' > $FILE
cat >> $FILE <<EOF
# FEAT version number
...
# Total volumes
set fmri(npts) ${NUMINPUTCOPESALL}
...
set fmri(multiple) ${NUMINPUTCOPESALL}
...
for i in ${!HIGHLEVELEqR[@]}
do
highl=${HIGHLEVELEqR[$i]}
cat >> $FILE <<EOF
${highl}
EOF
done
...
for i in ${!HIGHLEVELEqInd[@]}
do
highl=${HIGHLEVELEqInd[$i]}
cat >> $FILE <<EOF
${highl}
EOF
done
...
set fmri(conname_real.1) "EqualR>EqualInd"
...
set fmri(conname_real.2) "EqualInd>EqualR"
